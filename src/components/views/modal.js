...
export default function Modal({open, title, message, actionPositive, actionNegative}) {

    const cancelButtonRef = useRef(null)

    return (
        <Transition show={open ? true : false} as={Fragment}>
            <Dialog as="div" className="relative z-10" initialFocus={cancelButtonRef} onClose={actionPositive.action}>
                <Transition.Child
                    as={Fragment}
                            ...
                            <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                ...
                                <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                    <button
                                        ...
                                        onClick={actionNegative.action}
                                    >
                                        {actionNegative.title}
                                    </button>
                                    <button
                                        type="button"
                                        className="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
                                        onClick={actionPositive.action}
                                        ref={cancelButtonRef}
                                    >
                                        {actionPositive.title}
                                    </button>
                                </div>
                            </Dialog.Panel>
                        </Transition.Child>
